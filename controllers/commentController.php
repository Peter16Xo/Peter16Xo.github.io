...
require_once "./models/dao/commentDAO.php";

class commentController extends conexion {
    ...
    public function add_comment_controller() {
        ...
        $commentDTO = new commentDTO($codeClass, $comment, $AttFinalName, $typeUser, $codeUser, $dateNowC);

        if ($this->commentDAO->add_comment_model($commentDTO)) {
            $dataAlert = [
    ...
    public function delete_comment_controller($code, $codeU, $urls) {
        ...
        if ($data1['Codigo'] == $codeU) {
            if ($this->commentDAO->delete_comment_model($code)) {
                if ($data1['Adjunto'] != "") {
