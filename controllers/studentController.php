...
require_once "./models/dao/studentDAO.php";

class studentController extends conexion {
        ...
        if ($password1 != "" || $password2 != "") {
            if ($password1 == $password2) {
                ...
                if ($query1->rowCount() <= 0) {
                    ...
                    $studentDTO = new studentDTO($code, $name, $lastname, $email);

                    if (self::save_account($dataAccount) && $this->studentDAO->add_student_model($studentDTO)) {
                        $dataAlert = [
    ...
    public function data_student_controller($Type, $Code) {
        ...
        if ($studentdata = $this->studentDAO->data_student_model($data)) {
            return $studentdata;
        } else {
            $dataAlert = [
                "title" => "¡Ocurrió un error inesperado!",
    ...
    public function delete_student_controller($code) {
        ...
        if (self::delete_account($code) && $this->studentDAO->delete_student_model($code)) {
            $dataAlert = [
    ...
    public function update_student_controller() {
        ...
        if ($this->studentDAO->update_student_model($studentDTO)) {
            $dataAlert = [
