...
class adminController extends conexion {
        ...
        if ($password1 != "" || $password2 != "") {
            if ($password1 == $password2) {
                ...
                if ($query1->rowCount() <= 0) {
                    ...
                    $dataAdmin = new adminDTO($code, $name, $lastname);

                    if (self::save_account($dataAccount) && $this->adminDAO->addAdmin($dataAdmin)) {
                        $dataAlert = [
    ...
    public function data_admin_controller($Type, $Code) {
        ...
        if ($admindata = $this->adminDAO->getAdminData($data)) {
            return $admindata;
        } else {
            $dataAlert = [
                "title" => "¡Ocurrió un error inesperado!",
    ...
    public function delete_admin_controller($code) {
        ...
        if ($this->adminDAO->deleteAdmin($code)) {
            $dataAlert = [
    ...
    public function update_admin_controller() {
        ...
        if ($this->adminDAO->updateAdmin($dataAdmin)) {
            $dataAlert = [
