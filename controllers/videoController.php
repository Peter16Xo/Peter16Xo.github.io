<?php
	if($actionsRequired){
		require_once "../models/dao/videoDAO.php";
	}else{ 
		require_once "./models/dao/videoDAO.php";
	}
	
	class videoController extends videoDAO{

		/*----------  Add Video Controller  ----------*/
 public function add_video_controller() {
        if (!isset($_POST['title'], $_POST['teacher'], $_POST['date'], $_POST['code'], $_POST['description'])) {
            $dataAlert = [
        ...
        // Split the teacher value to get both the course ID and professorName
        list($courseId, $professorName) = explode('|', $_POST['teacher']);

        $date = self::clean_string($_POST['date']);
        $video = $_POST['code'];
        $description = $_POST['description'];

        $AttMaxSize = 5120; // Max size in KB
        ...
        foreach ($_FILES["attachments"]['tmp_name'] as $key => $tmp_name) {
            if ($_FILES["attachments"]["name"][$key]) {
                ...
                if (in_array($AttType, ["image/jpeg", "image/png", "application/msword", "application/vnd.ms-powerpoint", "application/pdf", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.openxmlformats-officedocument.presentationml.presentation"])) {
        ...
        if ($query1->rowCount() >= 1) {
            ...
            $data = [
                ...
                "Fecha"       => $date,
                "Titulo"      => $title,
                "Tutor"       => $professorName, // Store the professor's name
