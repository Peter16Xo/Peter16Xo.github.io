...
<?php 
    ...
    if(isset($_POST['title']) && isset($_POST['description'])){
        echo $insClass->add_video_controller();
    }

    $courses = $insCourse->get_all_courses();
// Convert the PHP array to JSON
$coursesJson = json_encode($courses);

// Output a JavaScript snippet to log the data
echo "<script>console.log('Courses Data: ', $coursesJson);</script>";
?>
<div class="container-fluid">
	<div class="row">
		<div class="col-xs-12">
										...
										<div class="form-group label-floating">
											...
											<select class="form-control" name="teacher" required>
												...
												foreach ($courses as $course) {
													$professorName = $course['profesorNombre'] . ' ' . $course['profesorApellido'];
													$courseValue = $course['id'] . '|' . $professorName;
													echo "<option value='" . $courseValue . "'>" . $course['Nombre'] . " - " . $professorName . "</option>";
												}
												?>
											</select>
										</div>
